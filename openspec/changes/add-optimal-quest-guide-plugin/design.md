...
Branding update: the plugin is named **QuestCape** in its descriptor, sidebar tooltip/header and Hub metadata; the Gradle project/distribution is `questcape`. The package and Gradle group are `com.questcape`, with `QuestCapePlugin`, `QuestCapeConfig` and `QuestCapeLauncher` as the plugin, configuration and development entry points. Keep the existing `optimalquestguide` configuration group and data directory so settings and account progress remain available. This package/class refactor supersedes the earlier decision to retain the Java entry-point name.
