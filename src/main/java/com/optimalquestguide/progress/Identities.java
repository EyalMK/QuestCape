...
public final class Identities
{
    ...
    static
    {
        for (Quest quest : Quest.values()) QUESTS.put(normalize(quest.getName()), quest.name());
    ...
    private static void alias(String source, String target)
    {
        ...
        if (id != null) QUESTS.put(normalize(source), id);
    }
    ...
    public static String quest(String title) { return QUESTS.get(normalize(title)); }
    public static String skill(String name)
    {
        String normalized = normalize(name);
        if (normalized.equals("runecrafting")) normalized = "runecraft";
        if (normalized.equals("defense")) normalized = "defence";
        for (Skill skill : Skill.values())
            if (normalize(skill.getName()).equals(normalized)) return skill.name();
        return null;
    }
}
