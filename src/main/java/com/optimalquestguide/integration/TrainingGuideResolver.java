...
import com.optimalquestguide.progress.Identities;
import java.io.*;
import java.net.URI;
import java.util.*;
import javax.inject.*;
...
public class TrainingGuideResolver
{
    ...
    @Inject public TrainingGuideResolver(BoundedHttp http)
    {
        this.http = http;
        try (InputStream in = TrainingGuideResolver.class.getResourceAsStream("/theoatrix.properties"))
        {
            if (in == null) throw new IllegalStateException("Missing packaged training links");
            urls.load(in);
        }
        catch (IOException e) { throw new IllegalStateException("Cannot read training links", e); }
    }
    ...
    public String label(String skill) { return resolve(skill).equals(FALLBACK) ? "Browse Theoatrix guides" : skill.replace('_', ' ') + " · Theoatrix guide"; }
    ...
    public String verifyDestination(String url) throws IOException
    {
        ...
        {
            if (!safe(url)) throw new IOException("Unsafe training destination. Use Browse Theoatrix guides.");
            BoundedHttp.Result response = http.get(url, Collections.emptyMap(), 4_000_000);
            if (response.getCode() == 200) return url;
            if (response.getCode() < 300 || response.getCode() > 399 || response.getLocation() == null)
                throw new IOException("Theoatrix returned HTTP " + response.getCode() + ". Retry or copy the guide link.");
            try { url = URI.create(url).resolve(response.getLocation()).toString(); }
            catch (IllegalArgumentException e) { throw new IOException("Invalid training redirect", e); }
        }
