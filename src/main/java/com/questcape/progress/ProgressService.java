...
public class ProgressService
{
    ...
    {
        if (observation == null || expectedSession != token()) return false;
        ...
        boolean unchanged = current != null && current.getScope().equals(observation.getScope())
            && current.getUsername().equals(observation.getUsername()) && current.getQuests().equals(observation.getQuests())
            && current.getLevels().equals(observation.getLevels()) && current.getActivities().equals(observation.getActivities());
        ...
        if (expectedSession != token()) return false;
        store.write("progress", next.getScope(), next);
        records.put(next.getScope(), next);
        synchronized (this)
        {
            if (expectedSession != session) return false;
            live = next;
            return true;
        }
    }
    public synchronized void logout() { live = null; session++; }
    public synchronized long token() { return session; }
    public AccountProgress current() { return live; }
    public AccountProgress viewed() { return live; }
    public void toggleManual(String expectedScope, GuideRow row, boolean checked) throws IOException
    {
        ...
        synchronized (this)
        {
            if (expectedSession == session && live == record) live = changed;
        }
    }
}
