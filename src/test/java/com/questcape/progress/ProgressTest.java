...
public class ProgressTest
{
    ...
    @Test public void persistenceRaceAccountAndModeIsolation() throws Exception
    {
        ...
        service.acceptLive(alice); long oldSession = service.token();
    ...
    @Test public void explicitSyncRefreshesTimestampAndPreservesManualChecks() throws Exception
    {
        ...
        GuideRow activity = new GuideParser().parse(GuideParserTest.table(GuideParserTest.activity("Unlock: boat"))).get(0);
        service.acceptLive(first); service.toggleManual(first.getScope(), activity, true);
    ...
    @Test public void logoutDoesNotWaitForDiskOrRepublishAnOldCharacter() throws Exception
    {
        JsonStore store = mock(JsonStore.class);
        ProgressService service = new ProgressService(store);
        ...
        doAnswer(i -> { entered.complete(null); release.get(5, java.util.concurrent.TimeUnit.SECONDS); return null; })
        ...
        try
        {
            ...
            java.util.concurrent.Future<Boolean> result = worker.submit(() -> service.acceptLive(account, session, true));
            entered.get(5, java.util.concurrent.TimeUnit.SECONDS);
            java.util.concurrent.CompletableFuture.runAsync(service::logout).get(1, java.util.concurrent.TimeUnit.SECONDS);
    ...
    @Test public void liveReaderUsesRealLevelsAndRequiresEstablishedProfile()
    {
        ...
        for (Skill skill : Skill.values()) if (skill != Skill.OVERALL) expectedSkills.add(skill.name());
        ...
        assertEquals(AccountProgress.QuestStatus.INCOMPLETE, new LiveProgressReader(client, config).read().getQuests().get("COOKS_ASSISTANT"));
        when(client.getWorldType()).thenReturn(EnumSet.of(WorldType.DEADMAN));
        assertNull(new LiveProgressReader(client, config).read());
        when(config.getRSProfiles()).thenReturn(List.of(new RuneScapeProfile("Alice", RuneScapeProfileType.DEADMAN, 42L, "rsprofile.a")));
        assertEquals("live:rsprofile.a:DEADMAN", new LiveProgressReader(client, config).read().getScope());
        when(config.getRSProfileKey()).thenReturn(null); assertNull(new LiveProgressReader(client, config).read());
        when(config.getRSProfileKey()).thenReturn("rsprofile.a");
        when(client.getGameState()).thenReturn(GameState.LOGIN_SCREEN); assertNull(new LiveProgressReader(client, config).read());
