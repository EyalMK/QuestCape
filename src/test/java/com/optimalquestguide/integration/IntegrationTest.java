...
public class IntegrationTest
{
    ...
    @Test public void noLaunchConfirmationMeansNoRememberedQuest()
    {
        ...
        verify(config, never()).setConfiguration(anyString(), anyString(), anyString(), anyString());
        assertTrue(new OptimalQuestGuideConfig() { }.resumeQuestOnLogin());
        assertEquals("optimalquestguide", OptimalQuestGuideConfig.class.getAnnotation(ConfigGroup.class).value());
    ...
    {
        com.optimalquestguide.progress.AccountProgress account = com.optimalquestguide.progress.ProgressTest.observation("live:A", "Alice", "STANDARD", Map.of(), Map.of("FIREMAKING", 1));
        com.optimalquestguide.guide.GuideRow row = new com.optimalquestguide.guide.GuideParser().parse(com.optimalquestguide.guide.GuideParserTest.table(
            com.optimalquestguide.guide.GuideParserTest.activity("Train Firemaking from level 1 to level 40"))).get(0);
        ...
        assertEquals(com.optimalquestguide.progress.Completion.State.INCOMPLETE, com.optimalquestguide.progress.Completion.of(row, account).getState());
    }
}
